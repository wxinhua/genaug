...
#     # for i, (rgb_image, depth) in enumerate(zip(rgb_images, depths)):
#     #     rgb_image = PIL.Image.fromarray(rgb_image)#cv2.cvtColor(rgb_image, cv2.COLOR_BGR2RGB)
#     #     #print(f"from trajectory rgb_image size：{rgb_image.size}")
...
#         image = PIL.Image.fromarray(new_rgb)
#         base_name, _ = os.path.splitext(image_name)
#         image.save(os.path.join(base_output_path, "aug_" + base_name + ".png"))
#         #read_h5files.process_aug_images_matrix(input_path, new_rgb, base_output_path)
...
#     image = PIL.Image.open("/nfsroot/DATA/IL_Research/will/datasets/real_franka/read_h5/241015_pick_bread_plate_1/success_episodes/1015_101547/camera_right/data/rgb_camera_right_41.jpg")
#     image_size = (np.array(image).shape[0], np.array(image).shape[1])
#     camera = camera_config(image_size, intrinsics, extrinsics)
...
#     depth = np.load("/nfsroot/DATA/IL_Research/will/datasets/real_franka/read_h5/241015_pick_bread_plate_1/success_episodes/1015_101547/camera_right/data/camera_right_41.npy") / 1000
...
#     for num_aug in range(1):
#         new_rgb, new_depth = genaug.augmentation(obj_names=['robot arm', 'bread', 'plate'], object=False, distractors=False, texture=False) #default: texture=True, object=False, distractors=True, background=True, table=True
#         PIL.Image.fromarray(new_rgb).save("/nfsroot/DATA/IL_Research/will/genaug-main/results/test_{}aug.jpg".format(num_aug))
...
class GenAug():
    ...
    def get_table(self, table_prompt, camera_config):
        ...
        init_depth[init_mask == 0] = -1
        init_table = PIL.Image.fromarray(init_table).resize((512, 512))
        init_table.save(os.path.join("/nfsroot/DATA/IL_Research/will/genaug-main", 'init_table.jpg'))
